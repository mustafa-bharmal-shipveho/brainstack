...
def run_dream_cycle():
    ...
    # continues — must NOT break the nightly dream cycle.
    consolidate_summary = ""
    try:
...
def run(brain_root=None, namespace="default", dry_run=False):
    ...
    # nightly dream cycle. The claim consolidator acquires its own
    # claims-log sentinel lock independently of the episodic lock.
    try:
        import consolidate  # local import — avoids cycles at module init
        import topic_keys
