...
def test_user_file_routed_to_personal_profile(tmp_path):
    src = tmp_path / "src"
    dst = tmp_path / "dst"
    make_flat_memory(src, {"user_azeem.md": SAMPLE_USER})

    run_migrate(src, dst)

    profile = dst / "memory" / "personal" / "profile" / "azeem.md"
...
def test_memory_md_index_rewritten(tmp_path):
    ...
    assert "personal/profile/azeem.md" in content
