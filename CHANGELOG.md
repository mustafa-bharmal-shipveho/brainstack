...
- Session digests: OMP (oh-my-pi) coding-agent transcripts at `~/.omp/agent/sessions/<slug>/<ts>_<uuid>.jsonl`
  ...
  defaults to `all` (`both` removed — nothing referenced it). Parse failures in OMP files are counted
  and logged instead of silently skipped, so transcript schema drift shows up in the sync log within a
  day instead of silently starving the brain.
